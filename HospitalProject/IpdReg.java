...
public class IpdReg extends JPanel {
	...
	public IpdReg() {
		...
		label.setIcon(new ImageIcon(IpdReg.class.getResource("/images/indoor.png")));
		...
		label_1.setIcon(new ImageIcon(IpdReg.class.getResource("/images/sidebar5.png")));
		...
		textArea.setLineWrap(true);
		textArea.setBounds(495, 555, 404, 70);
		add(textArea);
				...
				if(ipd_id.getText().trim().length()==0 ){
					JOptionPane.showMessageDialog(null,"Fill IPD ID.......");
						...
						try{
							...
							PreparedStatement psmt=con.prepareStatement("insert into Ipd_Reg values(?,?,?,?,?,?,?,?)");
							psmt.setString(1,ipd_id.getText());
							psmt.setInt(2,FollowupPatient.pid);
							psmt.setString(3,FollowupPatient.oid);
							psmt.setString(4,d1);
							psmt.setString(5,d2);
							psmt.setString(6,diagnosis.getText());
							psmt.setString(7,treatment.getText());
							psmt.setString(8,textArea.getText());
							psmt.executeQuery();
		...
		try {
			String s="select patient_id,patient_name,husband_name,father_name,surname from opd_reg where opd_id='"+FollowupPatient.oid+"'";
	...
	public static void create() {
		...
		f.getContentPane().add(new IpdReg());
